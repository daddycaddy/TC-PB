...
void AiFactory::AddDefaultCombatStrategies(Player* player, PlayerbotAI* const facade, Engine* engine)
{
	...
	{
		engine->addStrategies("warsong", "racials", "chat", "default", "dps", "potions", "conserve mana", "duel", "pvp", NULL);
	}
	else {
	...
	case CLASS_PRIEST:
        ...
        case CLASS_SHAMAN:
			if ((tab == 0) && (player->getLevel() > 9)) {
				...
				engine->addStrategies("caster", "bmana", "threat", "flee", NULL);
			}
        ...
        case CLASS_DRUID:
			if ((tab == 0) && (player->getLevel() > 9)) {
				...
				engine->addStrategies("caster", "threat", "flee", "dps assist", NULL);
			}
			break;
        case CLASS_HUNTER:
            engine->addStrategies("dps", "bdps", "threat", "dps assist", NULL);
	...
	if (sRandomPlayerbotMgr.IsRandomBot(player))
	{
		if (!player->GetGroup())
		{
			...
			if (player->getClass() == CLASS_DRUID && player->getLevel() < 10)
			{
				engine->addStrategies("caster", NULL);
			}
		}
	}
	else
